...
登录后可通过左侧“调用统计”查看真实网关数据，包括今日、本月及累计请求、成功与失败、首站命中率、自动切换次数、平均与 P95 耗时、近 7/30 天趋势，以及站点、模型、失败原因和接口排行。对兼容 New API 的站点，余额刷新还会尝试读取 `/api/user/topup/self` 最近一次成功充值记录，以订单的到账额度和实付人民币生成真实兑换率；成功网关调用会保存当时模型单价，并据此显示模型标价、折合市场价、实际成本和累计节省。没有充值记录、没有价格或没有用量的请求不会参与金额估算，也不会按 0 计算。
