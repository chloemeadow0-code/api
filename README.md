...
登录后可通过左侧“调用统计”查看真实网关数据，包括今日、本月及累计请求、成功与失败、首站命中率、自动切换次数、平均与 P95 耗时、近 7/30 天趋势，以及站点、模型、失败原因和接口排行。系统按成功调用的实际 Token 和模型价格计算已用额度价值，再用站点的美元/人民币换算率折算成人民币。对兼容 New API 的站点，余额刷新还会尝试读取 `/api/user/topup/self` 最近一次成功充值记录，以订单的到账额度和实付人民币生成真实兑换率；有充值记录时按实付汇率计算实际支付，没有充值记录时视为免费额度，实际支付为 0，已用额度价值全部计入累计节省。上方可信金额只统计调用时已经保存价格快照的新记录；旧日志会按实际模型名匹配当前目录估算，并单独显示在“旧记录估算（仅参考）”中，不会混入可信累计节省。只有总 Token 而没有输入/输出拆分时按输入单价近似估算。费用来源明细按站点和模型列出单价、Token 与贡献金额，便于定位异常价格；没有价格或没有 Token 用量的请求不会参与金额估算。
